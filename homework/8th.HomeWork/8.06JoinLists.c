 ...
 * from the second array, without repeating numbers, and arranged in 
 * increasing order. On the first input line, you are given the count of the 
 * elements of the first array. On the next line you are given the length of 
 * the second array.
 */
            ...
            int resultArrLength, int firstArrLength, int secondArrLength)
{
    ...
    for (i = 1, r = 1; i < firstArrLength; i++)
    {
        if (!arr_contains(resultArr, i, firstArr[i]))
        {
            resultArr[r] = firstArr[i];
            r++;
        }
    }
    ...
    for (r = 0; r < secondArrLength; r++)
    {
        if (!arr_contains(resultArr, i, secondArr[r]))
        {
            resultArr[i] = secondArr[r];
            i++;
        }
    }
}
